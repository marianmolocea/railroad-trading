  ...
  setInterval(() => {
    ...
    if (!(minutes % 15) && seconds > 0 && seconds <= 30) {
      watchList.forEach(async (currency) => {
        try {
          const priceData = await oanda.getPriceData(currency, 3, 'M15');
          let comparison = compareCandles(priceData[0], priceData[1]);

          if (comparison === 'trade') {
            let so = {
              currency,
              order: placeOrders.sellOrder(priceData[0], priceData[1]),
            };
            let bo = {
              currency,
              order: placeOrders.buyOrder(priceData[0], priceData[1]),
            };
            sellOrder.push(so);
            buyOrder.push(bo);
      ...
      sellOrder.forEach(async ({ currency, order }, index) => {
        try {
          ...
          if (order.entryPoint >= bid.c) {
            let response = await oanda.createMarketOrder(
              currency,
              -2000,
              order.tp,
              order.sl
            ...
            if (response.status === 201) {
              console.log(
                `New sell order on ${currency} successfully placed...`
              );
      ...
      buyOrder.forEach(async ({ currency, order }, index) => {
        try {
          ...
          if (order.entryPoint <= ask.c) {
            ...
            if (response.status === 201) {
              console.log(
                `New buy order on ${currency} successfully placed...`
              );
              buyOrder.splice(index, 1);
              sellOrder.splice(index, 1);
            }
